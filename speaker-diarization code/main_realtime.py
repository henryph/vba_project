...
def main(filename, config, Use_clustering_thread=False, Text_output = False):
    ...
    while i < audio_duration:
        ...
        kl.append(kbm_delay_s)
        dl.append(diff)   
        
        if i% 10 == 0:
            avg_kbm_delay_s = np.average(np.asarray(kl))
            avg_diff = np.average(np.asarray(dl))
            print(i, kl, avg_kbm_delay_s, dl, avg_diff)
            
            l = [str(x) for x in [i, avg_kbm_delay_s, avg_diff]]
            outf = open('./out/'+'realtime_test'+'.txt',"a+")    
            outf.writelines(','.join(l))
            outf.write('\n')
            outf.close()
    ...
    ll = [filename] + l
    outf = open('./out/'+'runtime'+'.rttm',"a")    
    outf.writelines(','.join(ll))
    outf.write('\n')
    outf.close()
    '''
...
if __name__ == "__main__":     
    ...
    with open('realtime_runtime.csv', 'w+', newline='') as csvfile:
        ...
        for filename in os.listdir(folder):
            ...
            l = main(filename, config,  Use_clustering_thread = True, Text_output = False)
