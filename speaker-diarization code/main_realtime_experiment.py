    ...
    while i < audio_duration:
        ...
        if kbm_t.Vg is not None:
            ...
            if Text_output:
                ...
                if i% 30 == 0:
                    print('output result for i=', i)
                    showName = filename.replace('.wav','')
                    getSegmentationFile('RTTM', frameshift,finalSegment, finalClustering, showName, 
                                        'result_'+str(i), './out/', '.rttm')
...
if __name__ == "__main__":     
    ...
    for filename in os.listdir(folder):
        if 'wav' in filename:   
            ...
            main(filename, config,  Use_clustering_thread = True, Text_output = True)
