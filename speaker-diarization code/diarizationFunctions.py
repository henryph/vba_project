...
def extractFeaturesFromSignal(y, sr, nfilters,ncoeff, NFFT, hop):
...
def extractFeatures(audioFile,framelength,frameshift,nfilters,ncoeff):
    ...
    #print('shape of input signal:', y.shape)
    
    frame_length_inSample=framelength*sr
    hop = int(frameshift*sr)
    NFFT=int(2**np.ceil(np.log2(frame_length_inSample)))
    '''
    print('number of samples in one frames: ', frame_length_inSample)
    print('hop(number of samples between successive frames):', hop)
    print('length of the FFT window:', NFFT)
    print('number of mel bands to generate:',nfilters)
    print('lowest frequency:', 20)
    print('highest frequency:', 7600)
    '''
    if sr>=16000:
        features=librosa.feature.mfcc(y=y, sr=sr,dct_type=2,n_mfcc=ncoeff,n_mels=nfilters,n_fft=NFFT,hop_length=hop,fmin=20,fmax=7600).T
    else:
...
def getSADfromSignal(y, sr, frameshift, nFeatures):
    va_framed = py_webrtcvad(y, fs=sr, fs_vad=sr, hoplength=30, vad_mode=0)
    segments = get_py_webrtcvad_segments(va_framed,sr)
    ...
    for i in range(segments.shape[0]):
        ...
        it = np.round(segments[i][0] / frameshift)
        et = np.round(segments[i][1] / frameshift)
...
def getSegmentTable(mask, speechMapping, wLength, wIncr, wShift):
    ...
    #print('nSegs from the mask:', nSegs)
...
def trainKBM(data, windowLength, windowRate, kbmSize):  
    ...
    # Calculate number of gaussian components in the whole gaussian pool, i.e. the poolSize
    ...
    for i in range(numberOfComponents):
        ...
        #print(i, i*windowRate, i*windowRate+windowLength, likelihoodVector[i])        
...
def getLikelihoodTable(data,gmPool,kbm):
    ...
    #print('kbm shape:', kbm.shape)
    #print('gmpool shape:', len(gmPool))
    ...
    #print('logLikelihoodTable shape:', logLikelihoodTable.shape)
...
def reselectBestClustering(ClusteringTable, bestClusteringID, prevC, cvT, mode = 1):
    

    from scipy.spatial.distance import cdist  
    
    print(ClusteringTable.shape)
    numberOfC = np.size(ClusteringTable, axis = 1)
    
    
    BC = ClusteringTable[:,bestClusteringID.astype(int)-1]
    ...
    if mode == 1:
        if bestClusteringID > 1 and bestClusteringID < numberOfC:
            ...
            BC = np.asarray(rearrangeClusterID(BC))
            C1 = np.asarray(rearrangeClusterID(C1))
            C2 = np.asarray(rearrangeClusterID(C2))
            prevC = np.asarray(prevC)
            
            diff = len(prevC) - len(BC)
            if diff < 0:
                C1_temp = C1[:diff]
                C2_temp = C2[:diff]
                BC_temp = BC[:diff]
            ...
            d1 = np.mean(np.square(C1_temp-prevC))
            db = np.mean(np.square(BC_temp-prevC))
            d2 = np.mean(np.square(C2_temp-prevC))

            l = [C1, BC, C2]

            minidx = np.argmin([d1, db, d2])
            print([d1, db, d2], minidx, np.unique(db),'-->', np.unique(l[minidx]))
            print(list(l[minidx]))
            return list(l[minidx])
        ...
        for i in [bestClusteringID - 1, bestClusteringID, bestClusteringID + 1]:
            ...
            for j in np.arange(np.size(clusterIDs,0)):
                ...
                distances = cdist(meanVector,cvT[clusterIDsIndex,:][0],'cosine')
...
def rearrangeClusterID(Clustering):
    ...
    # remove the 1-segment change
    len_c = len(Clustering)
    for i, c in enumerate(Clustering):
        if i > 0 and i < len_c -1:
            if c != Clustering[i-1] and c!= Clustering[i+1]:
...
def getSegResultForPlot(frameshift,finalSegmentTable, finalClusteringTable):
    ...
    for spk,timeDicts in speakerSlice.items():
        ...
        for timeDict in timeDicts:
            s = str(timeDict['start']/1000)
            e = str(timeDict['stop']/1000)
            print(s+' ==> '+e)
    '''
            
    return speakerSlice

def getSegResultForPlotlater(frameshift,finalSegmentTable, finalClusteringTable, showName, end_time, running_time):
    numberOfSpeechFeatures = finalSegmentTable[-1,2].astype(int)+1
    solutionVector = np.zeros([1,numberOfSpeechFeatures])
    for i in np.arange(np.size(finalSegmentTable,0)):
        solutionVector[0,np.arange(finalSegmentTable[i,1],finalSegmentTable[i,2]+1).astype(int)]=finalClusteringTable[i]
