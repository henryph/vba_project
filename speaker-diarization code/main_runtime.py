...
if __name__ == "__main__":     
    ...
    '''
    with open('runtime_stat.csv', 'a+', newline='') as csvfile:
    ...
    with open('runtime_stat_median.csv', 'a+', newline='') as csvfile:
        twriter = csv.writer(csvfile, delimiter=',')
        twriter.writerow(['filename','feature_t', 'SAD_t', 'seg_t', 'KBM_t','BKCV_t', 'clustering_t', 'reseg_t', 
                          'total', 'duration', 
                          'feature_RF', 'SAD_RF', 'seg_RF', 'KBM_RF', 'BKCV_RF', 'clustering_RF', 'reseg_RF',
    ...
    for filename in os.listdir(folder):
        ...
        medIdx = all_totalruntime.index(np.percentile(all_totalruntime,50,interpolation='nearest'))
        minIdx = all_totalruntime.index(min(all_totalruntime))
        
        #print(all_totalruntime, ' : ', medIdx, '-->', all_totalruntime[medIdx])
        print(all_totalruntime)
        
        with open('runtime_stat_median2.csv', 'a+', newline='') as csvfile:
            twriter = csv.writer(csvfile, delimiter=',')
            twriter.writerow(all_timelist[medIdx])
        
        with open('runtime_stat2.csv', 'a+', newline='') as csvfile:
            twriter = csv.writer(csvfile, delimiter=',')
            twriter.writerow(all_timelist[minIdx])
